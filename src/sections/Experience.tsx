    ...
    <AnimatedSection id="experiencia">
      <div className="container">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="absolute left-4 top-0 hidden h-full w-px bg-gradient-to-b from-cyan-300/60 via-violet-300/30 to-transparent sm:block" />
          {experience.map((item) => (
            <Card key={item.company} className="relative ml-0 sm:ml-12">
              <span className="absolute -left-[3.2rem] top-8 hidden h-4 w-4 rounded-full border-4 border-background bg-cyan-300 shadow-glow sm:block" />
              <CardContent className="p-6 sm:p-8">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="font-display text-2xl font-semibold text-white">{item.company}</p>
                    <p className="mt-1 text-cyan-100">{item.role}</p>
                  </div>
